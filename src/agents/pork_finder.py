...
class PorkFinder(AgentBase):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "CRITICAL FRAMING: distinguish ordinary class-of-recipient appropriations "
            "(e.g. '$10B for hazardous fuels reduction', '$3B for urban forestry programs') "
            "from real earmark patterns. Class-of-recipient appropriations should be "
            "OMITTED. But when a provision names a SPECIFIC person, organization, city, "
            "county, university, museum, or facility tied to a dollar amount, FLAG IT. "
            "The right number of flags depends on the bill: appropriations bills routinely "
            "carry many earmarks, while reconciliation/policy bills carry fewer. Don't "
            "self-censor: if you see ten clear named-recipient patterns, return ten. If "
            "you see zero, return zero. The signal is the structural pattern, not the count.\n\n"
            "PATTERN DEFINITIONS WITH EXAMPLES:\n\n"
            "1. named-recipient: bill names a specific entity OR a specific person "
            "(by name, role, or relationship). ALWAYS requires a non-empty recipient field. "
            "Includes: companies, universities, museums, non-profits, named individuals, "
            "named heirs/widows of named individuals, named labor unions, named tribes.\n"
            "   Flag: '$5,000,000 to the Lawrence Welk Museum in Strasburg ND, for renovation'\n"
            "         -> recipient='Lawrence Welk Museum'\n"
            "   Flag: 'Payment to Ramona Grijalva, widow of Raul M. Grijalva, $174,000.'\n"
            "         -> recipient='Ramona Grijalva (widow of Raul M. Grijalva)'\n"
            "   Flag: '$2,000,000 for the John F. Kennedy Center for the Performing Arts'\n"
            "         -> recipient='John F. Kennedy Center for the Performing Arts'\n"
            "   Flag: '$15,000,000 for research at the University of Mississippi'\n"
            "         -> recipient='University of Mississippi'\n"
            "   NON-flag: '$10B for hazardous fuels reduction projects' -> OMIT\n"
            "   NON-flag: 'grants to surviving spouses of deceased Members of Congress'\n"
            "             without naming any spouse -> OMIT (class-of-recipient)\n\n"
            ...
            "- Empty items[] is correct when the chunk genuinely has no flagged patterns. "
            "But if you see ANY of: a specific person/entity/place tied to a dollar amount, "
            "a sole-source clause, or an oddly-precise dollar figure, FLAG IT. When in "
            "doubt, flag with confidence='low' and let downstream review filter. The "
            "downside of missing real pork is much worse than over-flagging at low "
            "confidence.\n\n"
            "Return a single JSON object matching the requested schema. Cap items at 25."
        )
